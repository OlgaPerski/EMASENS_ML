...
write_rds(df_model_fit_hybrid, here("data", "hybrid models (sensor data)", "df_model_fit_hybrid.rds"))

# summarise model performance for each individual -------------------------

df_model_fit_hybrid <- read_rds(here::here("data", "hybrid models (sensor data)", "df_model_fit_hybrid.rds"))
...
sum(hybrid_to_group$hybrid_preferred == TRUE)/20*100
sum(hybrid_to_group$hybrid_preferred == FALSE)/20*100

hybrid_to_group_plot_sens <- hybrid_to_group %>%
  ggplot() +
...
if(!file.exists(here("outputs", "hybrid models (sensor data)", "hybrid_to_group_plot_sens.png"))) ggsave(hybrid_to_group_plot_sens, 
                                                                                                         filename = here("outputs", "hybrid models (sensor data)", "hybrid_to_group_plot_sens.png"),
