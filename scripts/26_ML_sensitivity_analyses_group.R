...
analytic_sample_clean <- analytic_sample_1 %>%
  mutate(sex = as.factor(sex),
         ...
         time_of_day = as.factor(case_when(EMA_hour >= 5 & EMA_hour < 11 ~ "morning",
                                           ...
                                           TRUE ~ "night")),
         cravings_binary = as.factor(case_when(craving_imp >6 ~ "high",
                                               TRUE ~ "low")),
         cravings_binary_lagged = as.factor(lead(cravings_binary)))

analytic_sample_clean <- write_rds(analytic_sample_clean, here("data", "sensitivity analyses", "analytic_sample_clean.rds"))

exclude <- c("nr", "id", "EMA_id", "EMA_hour", 
             "lapse_event_imp", "lapse_lagged", 
             "craving_imp", "cravings_binary",
             "nrt_no_presc", "nrt_presc", "zyban", "varen",
...
train_data %>%
  count(cravings_binary_lagged) %>%
  mutate(prop = n/sum(n)) # check target class balance

# set up recipe -----------------------------------------------------------

rec <- recipe(cravings_binary_lagged ~ ., data = train_data) %>%
  step_dummy(all_nominal(), -all_outcomes()) %>%
  step_normalize(age, cpd) %>%
  step_downsample(cravings_binary_lagged, skip = TRUE) # manages class imbalances through downsampling and skips this step during testing
...
vip_rf <- vip_rf$data %>%
  mutate(Variable = str_remove(Variable, "_imp"),
         ...
         Variable = str_replace(Variable, "Motivation to_stop_i.really.want.to.stop.smoking.and.intend.to.in.the.next.month", 
                                "Motivation to stop - I really want to stop smoking and intend to in the next month"),) %>%
  arrange(-Importance) %>%
...
write_rds(vip_rf, here("data", "sensitivity analyses", "group_vip_rf.rds"))

if(!file.exists(here("outputs", "sensitivity analyses", "group_vip_rf.png"))) ggsave(vip_rf, filename = here("outputs", "sensitivity analyses", "group_vip_rf.png"), 
...
svm_fit <- workflow() %>%
  ...
  add_formula(cravings_binary_lagged ~ .) %>%
  fit(juice(rf_prep)) 

vip_svm <- svm_fit %>%
  extract_fit_parsnip() %>%
  vip(method = "permute", 
      target = "cravings_binary_lagged", metric = "auc", reference_class = "high",
...
vip_svm_plot <- vip_svm_prep %>%
  mutate(Variable = str_remove(Variable, "_imp"),
         Variable = str_replace(Variable, "_", " "),
         Variable = str_to_sentence(Variable),
         Variable = str_replace(Variable, "Ethnicity white..any.white.background.", 
                                "Ethnicity - White"),
         Variable = str_replace(Variable, "Pharma support_yes", "Pharmacological support"),
         Variable = str_replace(Variable, "Beh support_yes", "Behavioural support")) %>%
  arrange(-Importance) %>%
  mutate(Variable = fct_inorder(Variable)) %>%
  ggplot() +
  geom_col(aes(x = Importance, y = fct_rev(Variable)), fill = "black") +
...
write_rds(vip_svm_plot, here("data", "sensitivity analyses", "group_vip_svm_plot.rds"))

if(!file.exists(here("outputs", "sensitivity analyses", "group_vip_svm.png"))) ggsave(vip_svm_plot, filename = here("outputs", "sensitivity analyses", "group_vip_svm.png"), 
...
vip_elnet <- vip_elnet$data %>%
  mutate(Variable = str_replace(Variable, "ethnicity_Other.ethnic.group..i.e...Arab.", "Ethnicity - Other ethnic group"),
         Variable = str_replace(Variable, "motivation_to_stop_I.really.want.to.stop.smoking.and.intend.to.in.the.next.month", 
                                "Motivation to stop - I really want to stop smoking and intend to in the next month"),
         Variable = str_replace(Variable, "ethnicity_Black..Black.British..Caribbean.or.African..any.Black..Black.British.or.Caribbean.background.",
         ...
         Variable = str_replace(Variable, "motivation_to_stop_I.want.to.stop.smoking.but.haven.t.thought.about.when",
                                "Motivation to stop - I want to stop smoking but haven't thought about when"),
         Variable = str_replace(Variable, "age", "Age"),
         Variable = str_replace(Variable, "beh_support_yes", "Behavioural support")) %>%
  arrange(-Importance) %>%
  mutate(Variable = fct_inorder(Variable)) %>%
  ggplot() +
  geom_col(aes(x = Importance, y = fct_rev(Variable)), fill = "black") +
...
write_rds(vip_elnet, here("data", "sensitivity analyses", "group_vip_elnet.rds"))

if(!file.exists(here("outputs", "sensitivity analyses", "group_vip_elnet.png"))) ggsave(vip_elnet, filename = here("outputs", "sensitivity analyses", "group_vip_elnet.png"), 
                                                                                                 dpi = 320, height = 8, width = 10)
...
write_rds(vip_xgb, here("data", "sensitivity analyses", "group_vip_xgb.rds"))

if(!file.exists(here("outputs", "sensitivity analyses", "group_vip_xgb.png"))) ggsave(vip_xgb, filename = here("outputs", "sensitivity analyses", "group_vip_xgb.png"), 
                                                                                               dpi = 320, height = 8, width = 10)
...
if(!file.exists(here("outputs", "sensitivity analyses", "group_combined_vip.png"))) save_plot(combined_vip, filename = here("outputs", "sensitivity analyses", "group_combined_vip.png"), 
...
roc_rf <- final_rf_res %>%
  collect_predictions() %>%
  select(.pred_high, cravings_binary_lagged)

roc_rf_obj <- roc(roc_rf$cravings_binary_lagged, roc_rf$.pred_high)
auc(roc_rf_obj)
...
roc_svm_obj <- roc(roc_svm$cravings_binary_lagged, roc_svm$.pred_high)
auc(roc_svm_obj)
...
roc_elnet_obj <- roc(roc_elnet$cravings_binary_lagged, roc_elnet$.pred_high)
auc(roc_elnet_obj)
...
roc_xgb_obj <- roc(roc_xgb$cravings_binary_lagged, roc_xgb$.pred_high)
auc(roc_xgb_obj)
...
flextable(table_performance) %>% 
  theme_vanilla() %>% 
  save_as_docx(path = here("outputs", "sensitivity analyses", "table_performance.docx"))

# save specs for best-performing model --------------------------------

# although the RF and XGB had identical AUCs, the RF had slightly higher specificity
# therefore, the RF was selected

write_rds(final_rf, here("data", "sensitivity analyses", "final_rf.rds"))
...
collect_predictions(final_rf_res) %>%
  conf_mat(cravings_binary_lagged, .pred_class)
...
rf_auc <-
  final_rf_res %>%
  collect_predictions() %>%
  roc_curve(cravings_binary_lagged, .pred_high) %>%
...
write_rds(combined_roc, here("data", "sensitivity analyses", "group_combined_roc.rds"))

if(!file.exists(here("outputs", "sensitivity analyses", "group_combined_roc.png"))) ggsave(combined_roc, filename = here("outputs", "sensitivity analyses", "group_combined_roc.png"),
                                                                                                    dpi = 320, height = 8, width = 10)
