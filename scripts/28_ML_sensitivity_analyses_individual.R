...
craving_count <- df %>%
  group_by(id, cravings_binary_lagged) %>%
  summarise(n_cravings = n()) %>%
  ungroup() %>%
  pivot_wider(names_from = cravings_binary_lagged, values_from = n_cravings) %>%
  filter(low > 5 & high > 5) %>%
  pull(id)

craving_count <- as.character(droplevels(craving_count)) # 31 participants available for analysis

rf_sample <- df %>%
  ungroup() %>%
  filter(id %in% craving_count) %>%
  group_by(id) %>%
  group_split()
...
run_rf_models <- function(x) {
  ...
  data_split <- initial_split(x, prop = .80, strata = cravings_binary_lagged)
  train_data <- training(data_split)
  test_data  <- testing(data_split)
  
  rf_rec <- recipe(cravings_binary_lagged ~ ., data = train_data) %>%
    step_dummy(all_nominal(), -all_outcomes()) %>%
    step_upsample(cravings_binary_lagged, skip = TRUE) # manages class imbalances in training sets
...
write_rds(individual_models_rf, here("data", "sensitivity analyses", "individual_models_rf.rds"))
...
run_svm_models <- function(x) {
  ...
  svm_rec <- recipe(cravings_binary_lagged ~ ., data = train_data) %>%
    step_dummy(all_nominal(), -all_outcomes()) %>%
...
write_rds(individual_models_svm, here("data", "sensitivity analyses", "individual_models_svm.rds"))
...
run_elnet_models <- function(x) {
  ...
  elnet_rec <- recipe(cravings_binary_lagged ~ ., data = train_data) %>%
    step_dummy(all_nominal(), -all_outcomes()) %>%
    step_upsample(cravings_binary_lagged, skip = TRUE) # manages class imbalances
...
write_rds(individual_models_elnet, here("data", "sensitivity analyses", "individual_models_elnet.rds"))
...
run_xgb_models <- function(x) {
  ...
  xgb_rec <- recipe(cravings_binary_lagged ~ ., data = train_data) %>%
    step_dummy(all_nominal(), -all_outcomes()) %>%
    step_upsample(cravings_binary_lagged, skip = TRUE) # manages class imbalances
...
write_rds(individual_models_xgb, here("data", "sensitivity analyses", "individual_models_xgb.rds"))

# extract model performance metrics ------------------------------

individual_models_rf <- read_rds(here("data", "sensitivity analyses", "individual_models_rf.rds"))
individual_models_svm <- read_rds(here("data", "sensitivity analyses", "individual_models_svm.rds"))
individual_models_elnet <- read_rds(here("data", "sensitivity analyses", "individual_models_elnet.rds"))
individual_models_xgb <- read_rds(here("data", "sensitivity analyses", "individual_models_xgb.rds"))

names(individual_models_rf) <- craving_count
names(individual_models_svm) <- craving_count
names(individual_models_elnet) <- craving_count
names(individual_models_xgb) <- craving_count
...
extract_model_characteristics <- function(account_ids = craving_count, rf_model = individual_models_rf, svm_model = individual_models_svm, elnet_model = individual_models_elnet, xgb_model = individual_models_xgb) {
...
if(!file.exists(here("outputs", "sensitivity analyses", "individual_model_performance_plot.png"))) ggsave(individual_model_performance_plot, 
                                                                                                                        filename = here("outputs", "sensitivity analyses", "individual_model_performance_plot.png"),
...
if(!file.exists(here("outputs", "sensitivity analyses", "individual_best_model_performance_stratified_plot.png"))) ggsave(individual_best_model_performance_stratified_plot, 
                                                                                                                          filename = 
                                                                                                                            here("outputs", "sensitivity analyses", "individual_best_model_performance_stratified_plot.png"),
                                                                                                                          dpi = 320, height = 8, width = 10)
...
if(!file.exists(here("outputs", "sensitivity analyses", "individual_best_model_performance_plot.png"))) ggsave(individual_best_model_performance_plot, 
                                                                                                               filename = here("outputs", "sensitivity analyses", "individual_best_model_performance_plot.png"),
                                                                                                               dpi = 320, height = 8, width = 10)
...
sum(individual_to_group$individual_preferred == TRUE)/31*100
sum(individual_to_group$individual_preferred == FALSE)/31*100
...
if(!file.exists(here("outputs", "sensitivity analyses", "individual_to_group_plot.png"))) ggsave(individual_to_group_plot, 
                                                                                                 filename = here("outputs", "sensitivity analyses", "individual_to_group_plot.png"),
                                                                                                 dpi = 320, height = 8, width = 10)

# plot 10 most important predictor vars (vip) by model type

extract_model_vip <- function(account_ids = craving_count, rf_model = individual_models_rf, svm_model = individual_models_svm, elnet_model = individual_models_elnet, xgb_model = individual_models_xgb) {
...
if(!file.exists(here("outputs", "sensitivity analyses", "individual_vip_by_model_type.png"))) ggsave(individual_vip_by_model_type, 
                                                                                                     filename = here("outputs", "sensitivity analyses", "individual_vip_by_model_type.png"), 
                                                                                                     dpi = 320, height = 8, width = 10)
...
if(!file.exists(here("outputs", "sensitivity analyses", "individual_vip_best_performing.png"))) ggsave(individual_vip_best_performing, 
                                                                                                       filename = here("outputs", "sensitivity analyses" , "individual_vip_best_performing.png"), 
