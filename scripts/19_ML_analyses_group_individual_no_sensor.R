...
write_rds(df_split_training, here("data", "group to individual models (no sensor data)", "df_split_training.rds"))

# fit best-performing group-level model to each individual's data ---------------------------

final_rf <- read_rds(here("data", "group models (no sensor data)", "final_rf.rds")) # get specs for best-performing model
...
write_rds(df_model_fit, here("data", "group to individual models (no sensor data)", "df_model_fit.rds"))

# summarise model performance for each individual -------------------------

df_model_fit <- read_rds(here("data", "group to individual models (no sensor data)", "df_model_fit.rds"))
...
write_rds(model_performance, here("data", "group to individual models (no sensor data)", "group_to_individual.rds"))

flextable(model_performance) %>% 
  theme_vanilla() %>% 
  save_as_docx(path = here("outputs", "group to individual models (no sensor data)", "model_performance.docx"))

model_performance <- read_rds(here("data", "group to individual models (no sensor data)", "group_to_individual.rds"))
